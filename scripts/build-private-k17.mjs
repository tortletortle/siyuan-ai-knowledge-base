...
for (const notePath of args) {
  ...
  sources.push({ source_id: sourceId, source_type: 'video-transcript', title, locator_type: 'note-section', trust: 'medium' });
  ...
  for (const match of items) {
    ...
    knowledge.push({ knowledge_id: id, title: `${title}｜${match[2].trim()}`, summary: itemText, body: itemText, topic: `K17/${title}`, status: 'active', source_ids: [sourceId], relations: [], evidence: [{ source_id: sourceId, locator: `note core knowledge ${match[1]}`, quote: itemText.slice(0, 240) }] });
  }
}
const queries = [];
