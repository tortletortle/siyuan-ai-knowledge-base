...
export function aggregateCoreBlocks(index) {
  ...
  for (const [rootId, blocks] of byRoot) {
    ...
    const representative = core.find((block) => /核心知识点|摘要/u.test(block.text)) ?? core[0];
    ...
    knowledge.push({ knowledge_id: `doc-core-${rootId}`, title: representative.title ?? `文档 ${rootId}`, summary: text.slice(0, 400), body: text, topic: representative.path ?? '思源', status: 'active', source_ids: [sourceId], relations: [], evidence: core.map((block) => ({ source_id: sourceId, locator: `block:${block.block_id}`, quote: block.text.slice(0, 240) })), doc_id: representative.doc_id, root_id: rootId, block_ids: blockIds });
  }
